...
* 代わりに `--lotteryall` を指定した場合は、認証アカウントの333の全競技者に対して当選ポイントを記録する。333以外は抽選。上のオプションと同様に、ポイント加算対象者をMySQLに待ちレコードとして保存する。ここでは実際にポイント加算はされない。このオプションは初回コンテストで全員当選にするために使用するとき用のものなのでもう必要がないので使わない方が良い。333以外の当選アルゴリズムに不具合があるが、もうメンテナンスされていない。
